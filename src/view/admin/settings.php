...
<div class="mdui-container-fluid">
	...
	<form action="" method="post">
		...
		<div class="mdui-textfield">
		  <h4>OneDrive起始目录(空为根目录)<small>例：仅共享share目录 /share （注：不要设置，该功能与白名单冲突）</small></h4>
		  <input class="mdui-textfield-input" type="text" name="onedrive_root" value="<?php echo $config['onedrive_root'];?>" disabled/>
		</div>


		<div class="mdui-textfield">
		  <h4 style="display: none;"><del>需要隐藏的目录<small> 不需要列出的目录(一行一个) 清空缓存后生效</small></del></h4>
                  <h4>白名单目录<small> 要列出的目录(一行一个) 清空缓存后生效 开头不用加<code>/</code></small></h4>
		  <textarea class="mdui-textfield-input" placeholder="输入后回车换行" name="onedrive_hide"><?=@$config['onedrive_hide'];?></textarea>
		  <del><small>这里是通配识别，就是存在以上字符文件夹一律会隐藏</small></del>
		</div>
