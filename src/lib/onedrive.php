<?php
	class onedrive{
		...
		static function dir($path="/"){
			...
			$items = array();
			$allow_list = explode(PHP_EOL,config('onedrive_hide'));
			if($path=="/") {
				if(is_array($allow_list)){
					foreach($allow_list as $allow_dir){
						$allow_dir = trim($allow_dir);
						if($allow_dir == ""){ return array(); }
						$items[]=array("name" => $allow_dir, "folder" => true);
					}
				}
				return $items;
			}
			if(is_array($allow_list)){
				foreach($allow_list as $allow_dir){
					$allow_dir = "/".trim($allow_dir);
					if(substr($path, 0, strlen($allow_dir)) === $allow_dir) {
