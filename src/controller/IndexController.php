...
class IndexController{
	...
	function file(){
		...
		if ($item['folder']) {//是文件夹
			...
			# Proxy
			$url = str_replace("<sharepoint_name>-my.sharepoint.com", "sharepointproxy.example.com", $url);
		}
		header('Location: '.$url);
	}
	...
	function dir(){
		...
		if($_SERVER["HTTP_ACCEPT"] == "application/psr.moe.lty.@microsoft.graph.onedrive.files-list+json") {
			header("Content-Type: application/psr.moe.lty.@microsoft.graph.onedrive.files-list+json");
			foreach($this->items as $i => $_) {
				unset($this->items[$i]["downloadUrl"]);
			}
			return json_encode($this->items);
		}
		if($_SERVER["HTTP_ACCEPT"] == "application/psr.moe.lty.@microsoft.graph.onedrive.files-list+xml") {
			$xml = new SimpleXMLElement('<Response/>');
			foreach($this->items as $item) {
				if($item["folder"]) { $item_xml = $xml->addChild("Folder"); }
					else { $item_xml = $xml->addChild("File"); }
				foreach($item as $key => $value) {
					if(($key == "folder") || ($key == "downloadUrl")) {
						// do nothing
					} else {
						$item_xml->addChild(ucfirst($key), $value);
					}
				}
			}
			header("Content-Type: application/psr.moe.lty.@microsoft.graph.onedrive.files-list+xml");
			return $xml->asXML();
		}
	...
	function show($item){
		...
		$_proxy_url = str_replace("luotianyi66ccff-my.sharepoint.com", "luotianyi-sharepointproxy.josephcz.xyz", $item['downloadUrl']);
		header('Location: '.$_proxy_url);
	}
	//缩略图
	...
	function is404(){
		...
		if($_SERVER["HTTP_ACCEPT"] == "application/psr.moe.lty.@microsoft.graph.onedrive.files-list+json") {
			$json = array("code" => 404, "desc" => "Not Found");
                        header("Content-Type: application/psr.moe.lty.@microsoft.graph.onedrive.files-list+json");
		...
		if($_SERVER["HTTP_ACCEPT"] == "application/psr.moe.lty.@microsoft.graph.onedrive.files-list+xml") {
			...
			$err->addChild("Code", 404);
			$err->addChild("Description", "Not Found");
                        header("Content-Type: application/psr.moe.lty.@microsoft.graph.onedrive.files-list+xml");
			echo $xml->asXML();
			die();
